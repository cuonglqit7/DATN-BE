...
<div class="mx-auto bg-white p-6 rounded-lg shadow-lg text-sm">
    ...
    <form action="{{ route('banners.update', $banner) }}" method="POST" enctype="multipart/form-data" class="needs-validation" novalidate>
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <input type="file" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-300" id="image_url" name="image_url" accept="image/*">
            <div class="mt-2">
                <img src="{{ asset($banner->image_url) }}" alt="{{ $banner->alt_text }}" class="w-32 h-32 object-cover">
            </div>
            @error('image_url')
        ...
        <div class="mb-4">
            ...
            <input type="text" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-300" id="alt_text" name="alt_text" placeholder="Mô tả ngắn gọn cho hình ảnh" value="{{ old('alt_text', $banner->alt_text) }}">
            @error('alt_text')
                <div class="text-red-500 mt-1 text-sm">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="url" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-300" id="link" name="link" placeholder="https://..." value="{{ old('link', $banner->link) }}">
            @error('link')
                <div class="text-red-500 mt-1 text-sm">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="number" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-300" id="position" name="position" min="0" value="{{ old('position', $banner->position) }}">
            @error('position')
                <div class="text-red-500 mt-1 text-sm">{{ $message }}</div>
            @enderror
        </div>
